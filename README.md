...
- **Sẵn sàng triển khai Render** — thêm `render.yaml` (Blueprint site tĩnh, phát hành thư
  mục gốc, không cần build); hướng dẫn từng bước ở mục *Đưa lên Render* bên dưới.
...
Trong thư mục đã có sẵn `render.yaml` — Blueprint cho static site phát hành thư mục gốc.

**Cách 1 — Blueprint (khuyên dùng):**
1. Đẩy thư mục này lên một repo GitHub.
2. Trên dashboard Render: **New + → Blueprint**, chọn repo đó — Render đọc `render.yaml`
   và tạo site `bong-x-chat` phát hành thư mục gốc.
3. Mở địa chỉ `https://bong-x-chat.onrender.com` (hoặc tên miền riêng bạn gắn sau).

**Cách 2 — Static Site thủ công:**
1. **New + → Static Site**, kết nối repo.
2. **Build Command**: để trống. **Publish Directory**: `.` (dấu chấm).
3. Bấm **Create Static Site**.
...
- Render tự lo HTTPS, nén và HTTP/2; site tĩnh không có bước build nên mỗi lần `git push`
  là bản mới được phát hành ngay.
